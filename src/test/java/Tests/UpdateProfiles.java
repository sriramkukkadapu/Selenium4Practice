...
public class UpdateProfiles {
 ...
 @Test public void f() throws InterruptedException {
  String data[][] = {
   {
    "sriramkukkadapu5c0@gmail.com",
    "Sriramk1"
   },
   {
    "ramyasha1989@gmail.com",
    "Sriramk1"
   },
   {
    "priyashadec1989@gmail.com",
    "Sriramk1"
   },
   {
    "sriramkukkadapu@gmail.com",
    "Sriramk1"
   }
  };
  ...
  try {
   ...
   for (i = 0; i < data.length; i++) {
    ...
    if (ui.equals("old")) old_flow(driver, data[i][0], data[i][1]);
    else new_flow(driver, data[i][0], data[i][1]);
    Thread.sleep(5000);
    driver.findElement(By.xpath("//em[@class='icon edit' and text()='Edit']")).click();
    Thread.sleep(3 * 1000);
 ...
 public String findUiOldOrNew(WebDriver driver) {
  String ui = null;
  if (driver.findElements(By.id("emailTxt")).size() > 0) return "old";
  else return "new";
 }
}